...
public interface ConfirmationTokenRepository extends JpaRepository<ConfirmationTokenModel,Integer>{
    @Query(value = "SELECT * FROM oriun_db.confirmation_token  WHERE confirmation_token = ?1 LIMIT 1",
    nativeQuery = true)
    ConfirmationTokenModel findByConfirmationToken(String confirmationToken);
    @Query(value = "SELECT * FROM oriun_db.confirmation_token  WHERE USER_NAME = ?1 LIMIT 1",
            nativeQuery = true)
    ConfirmationTokenModel findByUser(String user);
    @Modifying
    @Query(value = "UPDATE oriun_db.confirmation_token SET confirmation_token = ?2, CREATE_DATE= ?3 WHERE USER_NAME= ?1",
            nativeQuery = true)
